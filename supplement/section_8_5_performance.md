...
The Holme–Kim model extends Barabási–Albert preferential attachment with
a triangle-formation step that raises the clustering coefficient to levels
...
Holme–Kim networks we use m = 4 edges per new node and triangle
formation probability p = 0.5. To measure per-transaction latency we
sample 50 random source nodes from each generated graph, time Algorithm 1
(graph reduction) alone and then Algorithms 1 and 2 together, and divide
the total by 50, averaged over 3 independent seeds. For the block latency
...
At Bitcoin's approximate rate of 2000–3000 transactions per block and
...
clustering coefficient of 0.1–0.3 and a network diameter of 5–7 hops.
Our topology comparison experiment (measuring clustering over seeds at
|V| = 1 000–5 000) finds that the Doar topology yields clustering of
0.004–0.024, well below the real-network range, while the Holme–Kim
topology yields 0.21–0.24 — squarely within 0.1–0.3. Both models share
a similar average degree (~8) and approximate diameter of 5–6 hops,
matching the Bitcoin network. The Holme–Kim model is therefore a closer
structural proxy for real deployments. The fact that both models produce
nearly identical latency slopes confirms that the O(|V|) performance
guarantee is not sensitive to the level of local clustering, and the
ITFC overhead figures reported above remain valid across the realistic
range of cloud blockchain network topologies.
